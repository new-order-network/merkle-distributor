...
describe("MerkleDistributor", () => {
  ...
  describe("#token", () => {
    ...
    describe("Withdraw tokens by Owner", () => {
      ...
      it("withdraws specified amounts of tokens to address", async () => {
        ...
        expect(parseInt(await token.balanceOf(distributor.address))).to.equal(
          distributorInitialBalance - 10
        );
      ...
      it("withdraws all tokens to address", async () => {
        ...
        expect(parseInt(await token.balanceOf(userAddress))).to.equal(
          distributorInitialBalance
        );
      });
      ...
      context("inappropriate msg.sender", () => {
        it("reverts when withdrawing set amount of tokens to address", async () => {
          await expect(
            distributor.connect(user).withdrawToken(userAddress, 0)
          ).revertedWith("Ownable: caller is not the owner");
        });

        it("reverts when withdrawing all tokens to address", async () => {
          await expect(
            distributor.connect(user).withdrawAllTokens(userAddress)
          ).revertedWith("Ownable: caller is not the owner");
        });
      });
    });
    ...
    describe("Update merkle root", () => {
      ...
      context("with invalid request or params", () => {
        it("reverts when non-owner msg.sener attempts to update merkle root", async () => {
          await expect(
            distributor.connect(user).updateMerkleRoot(newTree.getHexRoot())
          ).revertedWith("Ownable: caller is not the owner");
        });
        ...
        it("reverts when claiming again with new amount less than previous amount in merkle tree", async () => {
          ...
          const newProof = newTree.getProof(
            1,
            wallet1.address,
            BigNumber.from(newAmount1)
          );
          await expect(
            distributor.claim(
              1,
      ...
      it("updates merkle root", async () => {
        await expect(
          distributor.connect(owner).updateMerkleRoot(newTree.getHexRoot())
        )
      ...
      it("claims with new merkle root: 0", async () => {
        ...
        const proof = newTree.getProof(
          0,
          wallet0.address,
          BigNumber.from(newAmount0)
        );
        await expect(
      ...
      it("claims again after updating merkle root: 0", async () => {
        ...
        expect(await token.balanceOf(wallet0.address)).to.equal(
          BigNumber.from(100)
        );
        await distributor.connect(owner).updateMerkleRoot(newTree.getHexRoot());
        const newProof = newTree.getProof(
  ...
  describe("two account tree", () => {
    ...
    it("sets #claimed", async () => {
      const proof0 = tree.getProof(0, wallet0.address, BigNumber.from(100));
      expect(await distributor.claimed(wallet0.address)).to.eq(0);
      expect(await distributor.claimed(wallet1.address)).to.eq(0);
      await distributor.claim(0, wallet0.address, 100, proof0, overrides);
      expect(await distributor.claimed(wallet0.address)).to.eq(100);
      expect(await distributor.claimed(wallet1.address)).to.eq(0);
    });
